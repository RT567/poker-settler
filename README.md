...
* [`src/poker/styles.cljs`](src/poker/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/poker/`](src/poker/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/poker/core.cljs): contains the SPA entry point, `init`
* [`test/poker/`](test/poker/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `poker` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/poker/config.cljs) defaults to `true` in
...
```clj
(ns poker.example
  (:require [poker.config :as config])
